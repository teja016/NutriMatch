...
// ── BLOOD REPORT PARSER (PDF.js — no API, no CORS) ───────────
...
// Clinical reference ranges & deficiency mapping
const MARKER_DEFS = [
  // ── NUTRITION MARKERS ──────────────────────────────────────
  { name:"Vitamin D",       aliases:["vitamin d","25-oh","25 oh","25-hydroxyvitamin","vit d","vitd"],   unit:"ng/mL", low:20,  high:100, deficiency:"Vitamin D" },
  { name:"Vitamin B12",     aliases:["vitamin b12","b12","cyanocobalamin","cobalamin"],                  unit:"pg/mL", low:200, high:900, deficiency:"Vitamin B12" },
  { name:"Folate",          aliases:["folate","folic acid","vitamin b9","b9"],                           unit:"ng/mL", low:3,   high:20,  deficiency:"Folate" },
  { name:"Vitamin C",       aliases:["vitamin c","ascorbic acid","vit c"],                               unit:"mg/dL", low:0.4, high:2.0, deficiency:"Vitamin C" },
  { name:"Serum Iron",      aliases:["serum iron","s. iron","iron serum","fe serum"],                    unit:"µg/dL", low:60,  high:170, deficiency:"Iron" },
  { name:"Ferritin",        aliases:["ferritin","serum ferritin"],                                       unit:"ng/mL", low:12,  high:300, deficiency:"Iron" },
  { name:"Calcium",         aliases:["calcium","serum calcium","ca serum","s. calcium"],                 unit:"mg/dL", low:8.5, high:10.5,deficiency:"Calcium" },
  { name:"Zinc",            aliases:["zinc","serum zinc","zn serum"],                                    unit:"µg/dL", low:60,  high:130, deficiency:"Zinc" },
  { name:"Magnesium",       aliases:["magnesium","serum magnesium","mg serum","s. magnesium"],           unit:"mg/dL", low:1.7, high:2.4, deficiency:"Magnesium" },
  // ── BLOOD COUNT ────────────────────────────────────────────
  { name:"Haemoglobin",     aliases:["haemoglobin","hemoglobin","hb","hgb"],                             unit:"g/dL",  low:12,  high:17.5,deficiency:"Iron" },
  { name:"WBC",             aliases:["wbc","white blood","leucocyte","leukocyte","tlc"],                 unit:"cells/µL", low:4000, high:11000, deficiency:null },
  { name:"RBC",             aliases:["rbc","red blood","erythrocyte","tlrbc"],                           unit:"million/µL", low:3.8, high:5.8, deficiency:null },
  { name:"Platelets",       aliases:["platelet","thrombocyte","plt"],                                    unit:"lakh/µL", low:1.5, high:4.0, deficiency:null },
  { name:"MCV",             aliases:["mcv","mean corpuscular volume"],                                   unit:"fL",    low:80,  high:100, deficiency:null },
  { name:"MCH",             aliases:["mch","mean corpuscular hemo"],                                     unit:"pg",    low:27,  high:33,  deficiency:null },
  { name:"Haematocrit",     aliases:["haematocrit","hematocrit","pcv","packed cell"],                    unit:"%",     low:36,  high:50,  deficiency:null },
  // ── THYROID ────────────────────────────────────────────────
  { name:"TSH",             aliases:["tsh","thyroid stimulating","thyrotropin"],                         unit:"mIU/L", low:0.4, high:4.0, deficiency:null },
  { name:"T3",              aliases:[" t3 ","triiodothyronine","free t3","ft3"],                          unit:"pg/mL", low:2.3, high:4.2, deficiency:null },
  { name:"T4",              aliases:[" t4 ","thyroxine","free t4","ft4"],                                 unit:"ng/dL", low:0.8, high:1.8, deficiency:null },
  // ── GLUCOSE / HbA1c ────────────────────────────────────────
  { name:"Blood Glucose (F)",aliases:["fasting glucose","blood glucose f","fbs","glucose fasting","fasting blood sugar"], unit:"mg/dL", low:70, high:100, deficiency:null },
  { name:"HbA1c",           aliases:["hba1c","glycated haemo","glycosylated","a1c"],                     unit:"%",     low:0,   high:5.7, deficiency:null },
  // ── LIPID PROFILE ──────────────────────────────────────────
  { name:"Total Cholesterol",aliases:["total cholesterol","cholesterol total","serum cholesterol"],      unit:"mg/dL", low:0,   high:200, deficiency:null },
  { name:"LDL",             aliases:["ldl","low density lipoprotein","ldl-c"],                           unit:"mg/dL", low:0,   high:130, deficiency:null },
  { name:"HDL",             aliases:["hdl","high density lipoprotein","hdl-c"],                          unit:"mg/dL", low:40,  high:999, deficiency:null },
  { name:"Triglycerides",   aliases:["triglyceride","tg ","tgl"],                                        unit:"mg/dL", low:0,   high:150, deficiency:null },
  // ── LIVER / KIDNEY ─────────────────────────────────────────
  { name:"ALT (SGPT)",      aliases:["alt","sgpt","alanine aminotransferase","alanine transaminase"],    unit:"U/L",   low:0,   high:40,  deficiency:null },
  { name:"AST (SGOT)",      aliases:["ast","sgot","aspartate aminotransferase","aspartate transaminase"],unit:"U/L",   low:0,   high:40,  deficiency:null },
  { name:"Creatinine",      aliases:["creatinine","serum creatinine","s. creatinine"],                   unit:"mg/dL", low:0.6, high:1.3, deficiency:null },
  { name:"Urea",            aliases:["urea","blood urea","serum urea"],                                  unit:"mg/dL", low:15,  high:45,  deficiency:null },
  { name:"Uric Acid",       aliases:["uric acid","serum uric"],                                          unit:"mg/dL", low:2.5, high:7.5, deficiency:null },
];

function getStatus(val, low, high) {
  const margin = 0.10;
  if (val < low * (1 - margin))       return "low";
  if (val < low)                       return "borderline_low";
  if (val > high * (1 + margin))      return "high";
  if (val > high)                     return "borderline_high";
  return "normal";
}

function parseNumberFromText(text, aliases) {
  const textL = text.toLowerCase();
  for (const alias of aliases) {
    // Look for alias followed by number(s) within ~120 chars
    const idx = textL.indexOf(alias);
    if (idx === -1) continue;
    const window = text.slice(idx, idx + 160);
    // Match numeric value (supports decimals and commas)
    const match = window.match(/[:\s]+([<>]?\s*[0-9][0-9,\.]*)/);
    if (match) {
      const raw = match[1].replace(/[<>\s,]/g, "");
      const num = parseFloat(raw);
      if (!isNaN(num) && num > 0) {
        // Also try to grab reference range
        const refMatch = window.match(/([0-9]+\.?[0-9]*)\s*[-–—to]+\s*([0-9]+\.?[0-9]*)/g);
        return { value: num, refText: refMatch ? refMatch[0] : null };
      }
    }
  }
  return null;
...
async function analyzeBloodReportWithAI(fileObj) {
  ...
  if (fileObj.type === "application/pdf") {
    rawText = await extractPDFText(fileObj);
  } else {
    // For images, do basic OCR hint via filename + return limited parse
    rawText = fileObj.name || "";
  }

  if (!rawText || rawText.trim().length < 30) {
    throw new Error("Could not read text from this file. Please upload a text-based PDF (not a scanned image).");
  }
  ...
  for (const def of MARKER_DEFS) {
    const result = parseNumberFromText(rawText, def.aliases);
    if (!result) continue;

    const { value, refText } = result;
    const status = getStatus(value, def.low, def.high);

    markers.push({
      name: def.name,
      value: `${value} ${def.unit}`,
      referenceRange: refText || `${def.low}–${def.high} ${def.unit}`,
      status,
      deficiency: def.deficiency,
    });

    if (def.deficiency && (status === "low" || status === "borderline_low")) {
      if (!deficiencies.includes(def.deficiency)) deficiencies.push(def.deficiency);
    }
  }

  if (markers.length === 0) {
    throw new Error("No blood markers found in PDF. Please ensure it is a text-based lab report (not a scanned image).");
  }

  const abnormal = markers.filter(m => m.status !== "normal");
  const summary = abnormal.length === 0
    ? `All ${markers.length} markers detected are within normal reference ranges.`
    : `${abnormal.length} of ${markers.length} markers are outside normal range. Key concerns: ${abnormal.slice(0,3).map(m=>m.name).join(", ")}.`;

  return { markers, deficiencies, summary };
}
        ...
        {!draft.bloodAnalysis&&!analyzing&&(
          <>
            <div className="br-zone" onClick={()=>fileRef.current?.click()}>
              ...
              <div style={{fontSize:10,color:"var(--mu)",fontWeight:400}}>Text-based PDF lab reports — tap to browse</div>
            </div>
